...
function getlistwithPage(){
	...
	content.push("<thead class='tableH'><td style='width:10%;text-align:center;'>课程ID</td><td style='width:20%;text-align:center;'>课程名称</td><td style='width:10%;text-align:center;'>授课老师</td><td style='width:10%;text-align:center;'>上课地点</td><td style='width:10%;text-align:center;'>教室可容纳人数</td><td style='width:10%;text-align:center;'>课程类型</td><td style='width:20%;text-align:center;'>上课时间</td><td style='width:10%;text-align:center;'>操作</td></thead>");
	content.push("<tbody>");
	$.ajax({
		type:"post",
		async:true,
		url:"StuCourseServlet?pageSize="+pageSize+"&pageNum="+pageNum+"&CourseName="+CourseName+"&CourseType="+CourseType,
		dataType:"json",
		success:function(result){
			...
			if(dateArr.length>0){
				for(var i =0;i<dateArr.length;i++){
					...
					content.push("<td style='text-align:center;'><a id='selectcoursebtn' href='StuAddCourseServlet?id="+dateArr[i].courseID+"&StudentID="+StudentID+"' style='display:inline-block;height:30px;width:50px;background-color:#2d6fb7;color:white;line-height:30px;disabled;'>选课</a></tb>");
					content.push("</tr>");
				}
			}
			content.push("</tbody>");
